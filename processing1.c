...
uint16_t lastValid = 512;     //10-bit mid-scale
uint16_t runningMean = 512;   //10-bit mid-scale
uint16_t prevAvg = 0;

uint16_t data[1];             //SPI RX buffer, holds 10-bit value
uint8_t uartData[1];
volatile uint8_t tx;          //rescaled 8-bit output to UART
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
    if (htim->Instance == TIM1) {
        ...
        if (tail != head) {
            ...
            //moving average of 2, still in 10-bit range (0-1023)
            uint16_t avg = (first + second) / 2;

            //outlier rejection threshold scaled for 10-bit range
            //(was 50 in 8-bit ~20% of 256, now 200 in 10-bit ~20% of 1024)
            ...
            //rescale 10-bit (0-1023) to 8-bit (0-255) by dropping 2 LSBs
            tx = (uint8_t)(avg >> 2);
...
int main(void)
{
  ...
  HAL_SPI_Receive_DMA(&hspi1, (uint8_t*)data, 1);
...
void HAL_SPI_RxCpltCallback(SPI_HandleTypeDef *hspi) {
    ...
    //store if buffer isn't full (data[0] is now uint16 holding the 10-bit value)
    if (next != tail) {
        buffer[head] = data[0];
        head = next;
    }
