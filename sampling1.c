...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
    if (htim->Instance == TIM2) {
        if (tail != head) {
            ...
            //get first sample (10-bit value stored in uint16)
        	spi_tx = buffer[tail];
            //set the tail to the next position
            tail = (tail + 1) % BUFF_SIZE;

            if (hspi1.State == HAL_SPI_STATE_READY) {
                //SPI is configured 16-bit DataSize, so size=1 sends one 16-bit word
                HAL_SPI_Transmit_IT(&hspi1, (uint8_t*)&spi_tx, 1);
            }
        }
    }
}
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc){
    ...
    if (next != tail) {
        //data[0] holds the raw 10-bit ADC value (0-1023)
        buffer[head] = (uint16_t)data[0];
        head = next;
    }
